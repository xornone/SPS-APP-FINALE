...
// Copie de RideForm.tsx pour la page de test /admin2test : le formulaire
// admin reel (components/RideForm.tsx) n'est volontairement pas touche.
// Seul ajout par rapport a l'original : le bouton "Importer depuis Strava"
// pres du champ lien Strava, qui appelle /api/admin2test/strava/import puis
// reutilise exactement le meme chemin que l'upload manuel d'un fichier
// .gpx (setGpxFile/setPreview/...) — voir handleStravaImport ci-dessous.
...
export function RideFormStravaTest({
  ride,
  connectedAdmins,
}: {
  ride?: Ride;
  ...
  const [stravaAdmin, setStravaAdmin] = useState(connectedAdmins[0] || "");
  ...
  // Recupere le trace directement depuis Strava (compte connecte de
  // `stravaAdmin`) plutot que de demander a l'admin d'exporter puis
  ...
  async function handleStravaImport() {
    if (!stravaAdmin || !stravaUrl.trim()) return;
    setStravaImporting(true);
    setStravaImportError("");
    try {
      const res = await fetch("/api/admin2test/strava/import", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ adminName: stravaAdmin, activityUrl: stravaUrl.trim() }),
      });
      ...
      <Field label="Lien Strava (optionnel)">
        ...
        {connectedAdmins.length > 0 && (
          <div className="mt-2 flex items-center gap-2">
            {connectedAdmins.length > 1 ? (
              <select
                value={stravaAdmin}
                onChange={(e) => setStravaAdmin(e.target.value)}
                className="input !w-auto flex-none py-2 text-xs"
              >
                {connectedAdmins.map((name) => (
              ...
              <span className="flex-none text-[11px] text-black/45 dark:text-white/45">{stravaAdmin}</span>
            )}
            <button
              type="button"
        ...
        {connectedAdmins.length === 0 && (
          <p className="mt-1.5 text-[11px] text-black/35 dark:text-white/35">
            Aucun compte Strava connecté — connecte Duc ou Aymeric depuis /admin2test pour activer l’import automatique.
          </p>
        )}
